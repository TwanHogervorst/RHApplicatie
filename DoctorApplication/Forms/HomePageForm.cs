    ...
    public partial class HomePageForm : Form
    {
        ...
        private void Client_OnClientListReceived(Dictionary<string, bool> clientList_)
        {
            this.Invoke((Action)delegate
            {
                ...
                {
                    PatientListViewUserControl patientList = new PatientListViewUserControl(client, userClient.Key);
                    patientList.OnPatientListViewItemClick += PatientList_OnUserSelected;
                    PatientTableLayoutPanel.Controls.Add(patientList);
                }

                Func<PatientListViewUserControl, bool> selectPredicate;
                ...
                this.PatientTableLayoutPanel.Controls.OfType<PatientListViewUserControl>().FirstOrDefault(selectPredicate)?.SelectUser();
            });
        }

        private void PatientList_OnUserSelected(string username)
        {
            foreach (PatientListViewUserControl plvuc in this.PatientTableLayoutPanel.Controls.OfType<PatientListViewUserControl>())
